...
int (xpm_draw_ignore_Block)(xpm_map_t xpm, uint16_t x, uint16_t y, unsigned int ignoredColor, int number) {
    ...
    const uint8_t font[10][14] = { // Meter maior, e dar fix na maneira de como desenham 
        {0x00, 0x00, 0x7E, 0xFF, 0xFF, 0x81, 0x81, 0xFF, 0xFF, 0x81, 0x81, 0xFF, 0x7E, 0x00}, 
        {0x00, 0x00, 0x00, 0x82, 0xFF, 0xFF, 0x80, 0x80, 0xFF, 0xFF, 0x82, 0x00, 0x00, 0x00}, 
        {0x00, 0x00, 0xE2, 0xF3, 0x9B, 0x8B, 0x89, 0x89, 0x8F, 0x8E, 0x00, 0x00, 0x00, 0x00}, 
        {0x00, 0x00, 0x42, 0xC3, 0x89, 0x89, 0x89, 0x89, 0xFF, 0x76, 0x00, 0x00, 0x00, 0x00}, 
        {0x00, 0x00, 0x1F, 0x11, 0x11, 0x11, 0xFF, 0xFF, 0x10, 0x10, 0x00, 0x00, 0x00, 0x00}, 
        {0x00, 0x00, 0x4F, 0xCF, 0x89, 0x89, 0x89, 0x89, 0xF9, 0x71, 0x00, 0x00, 0x00, 0x00}, 
        {0x00, 0x00, 0x7E, 0xFF, 0x89, 0x89, 0x89, 0x89, 0xFB, 0x72, 0x00, 0x00, 0x00, 0x00}, 
        {0x00, 0x00, 0x01, 0x01, 0xF1, 0xF9, 0x09, 0x09, 0x0F, 0x07, 0x00, 0x00, 0x00, 0x00}, 
        {0x00, 0x00, 0x76, 0xFF, 0x89, 0x89, 0x89, 0x89, 0xFF, 0x76, 0x00, 0x00, 0x00, 0x00}, 
        {0x00, 0x00, 0x4E, 0xDF, 0x91, 0x91, 0x91, 0x91, 0xFF, 0x7E, 0x00, 0x00, 0x00, 0x00}  
    };
   ...
   if (number < 0 ) {
    ...
    int num_width = 10; 
    int num_height = 14; 
    int centerX = x + (img.width - num_width) / 2;
    int centerY = y + (img.height - num_height) / 2;

    for (int i = 0; i < num_height; i++) {
        for (int j = 0; j < num_width; j++) {
            if (font[number][i] & (1 << (7 - (j / 2)))) {
                vg_draw_pixel(centerX + j, centerY + i, 0x000000, current_buffer); 
            }
        }
    }
